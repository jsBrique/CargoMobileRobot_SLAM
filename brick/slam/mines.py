...
from breezyslam.sensors import URG04LX,EAIX4
...
def transdegree(lst, k):

    return lst[k:] + lst[:k]

def load_data_EAI(datadir, dataset):
    ...
    while True:  
        ...
        toks = s.split()[0:] # ignore ''

        timestamp = int(toks[0])

        odometry = timestamp, int(toks[1]), int(toks[2])
                        
        lidar = transdegree([int(tok) for tok in toks[3:]],360)
...
def load_data(datadir, dataset):
    ...
    while True:  
        ...
        # print('timestamp=',timestamp,'odometry=',odometry)
        # print('lidar=',lidar,'!!!')
        timestamps.append(timestamp)
        scans.append(lidar)
        odometries.append(odometry)
...
class MinesLaser(EAIX4):
    
    def __init__(self):
        
        EAIX4.__init__(self, 0, 0)
